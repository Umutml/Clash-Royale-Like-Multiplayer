                    ...
                    if (runtime.mesh_lod_range != null && runtime.mesh_lod_range.Length != 0)
                    {
                        for (int i = 0; i < runtime.mesh_lod_range.Length; i++)
                        {
                            if (i % 2 == 0)
                            {
                                ...
                                EditorGUILayout.LabelField("Triangles Count: " + MantisLODEditorUtility.get_triangles_count_from_progressive_mesh(runtime.progressiveMesh, runtime.mesh_lod_range[i], i / 2).ToString());
                                GUILayout.Label(
